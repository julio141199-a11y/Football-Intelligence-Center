...
const DATABASES = {
  jobs: "jobs.json",
  pipelineOpportunities: "data/opportunities.json",
  chatOpportunities: "data/chat_opportunities.json",
...
function pipelineOpportunityToJob(item) {
  const detectedDate = text(item.detectedAt).slice(0, 10);
  const sourcePlatform = item.official ? "Official Website" : text(item.sourceType);
  return {
    id: item.id,
    dateAdded: validDate(detectedDate) ? detectedDate : "To verify",
    lastChecked: validDate(text(item.lastVerified)) ? item.lastVerified : detectedDate,
    status: item.status === "Verified Open" ? "Open" : item.status,
    priority: item.careerPriority || "Monitor",
    roleType: item.role,
    position: item.title,
    country: item.country,
    continent: item.region,
    organization: item.organisation,
    teamType: item.teamType || "To verify",
    deadline: item.deadline || "To verify",
    licenceRequirement: item.licenceRequirement || "To verify",
    afcALicenceFit: item.licenceNote || "To verify",
    applicationMethod: item.applicationMethod || "Open the official source and verify the application route.",
    contactPerson: item.contactPerson || "Not publicly listed",
    contactEmail: item.contactEmail || "Not Public",
    contactPhone: item.contactPhone || "Not Public",
    applicationLink: item.applicationLink || item.sourceUrl,
    sourcePlatform,
    sourceName: item.sourceName || item.organisation,
    sourceUrl: item.sourceUrl,
    officialSocialAccount: item.officialSocialAccount || "To verify",
    accuracyLevel: item.status === "Verified Open" ? "Verified from the supplied official source." : "To Verify",
    fitScore: item.fitScore || "To verify",
    fitReason: item.fitReason || item.licenceNote || "Profile fit requires review.",
    shortSummary: item.shortSummary || `${item.eventType || "Opportunity"} detected from the monitored source.`,
    details: item.details || item.title,
    recommendedAction: item.recommendedAction || "Verify the source before applying.",
    notes: item.notes || "Generated by the FIC opportunity pipeline."
  };
}
function vacancyToJob(item) {
  const statusMap = { NEW: "New", UPDATED: "Updated", CLOSING_SOON: "Closing Soon", CLOSED: "Closed", EXPIRED: "Expired", UNVERIFIED: "To Verify" };
  return {
    id: item.id, dateAdded: item.created_at?.slice(0, 10) || item.posted_date || "To verify",
    lastChecked: item.last_checked || "To verify", status: statusMap[item.status] || "To Verify",
    priority: "Monitor", roleType: item.role, position: `${item.organization} ${item.role}`,
    country: item.country, continent: item.region, organization: item.organization,
    teamType: item.team_level || "To verify", deadline: item.deadline || "To verify",
    licenceRequirement: item.licence_requirement || "To verify", afcALicenceFit: item.fit_assessment || "To verify",
    languageRequirement: item.language_requirement || "To verify",
    applicationMethod: item.application_url ? "Apply through the official application link." : "Use the verified official contact or source.",
    contactPerson: "Not publicly listed", contactEmail: item.contact_email || "Not Public",
    contactPhone: item.contact_phone || "Not Public", applicationLink: item.application_url || item.official_source_url,
    sourcePlatform: item.source_type || "Official Website", sourceName: item.organization,
    sourceUrl: item.official_source_url, officialSocialAccount: /instagram|facebook|linkedin|\bx\b/i.test(item.source_type || "") ? item.official_source_url : "To verify",
    accuracyLevel: item.accuracy_level || (item.status === "UNVERIFIED" ? "To Verify" : "Verified from supplied official source."),
    fitScore: item.fit_score || "To verify", fitReason: item.fit_assessment || "Profile fit requires review.",
    shortSummary: `${item.role} vacancy record for ${item.organization}.`, details: [item.contract_type, item.salary, item.housing, item.visa, item.flights].filter(Boolean).join(" · ") || "See official source.",
    recommendedAction: ["NEW", "UPDATED", "CLOSING_SOON"].includes(item.status) ? "Review and apply promptly." : "Monitor the official source.",
    notes: `Vacancy ID: ${item.id}`
  };
}
function mergeOpportunityFeeds() {
  ...
  merged.forEach((item) => {
    const key = safeUrl(item.sourceUrl)
      ? `${safeUrl(item.sourceUrl).replace(/\/$/, "").toLowerCase()}|${text(item.roleType).toLowerCase()}`
...
async function loadDatabases() {
  ...
  const entries = await Promise.allSettled(Object.entries(DATABASES).map(async ([key, url]) => {
      const response = await fetch(`${url}?v=${cacheVersion}`, { cache: "no-store" });
